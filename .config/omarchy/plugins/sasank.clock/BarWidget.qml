...
BarWidget {
  ...
  function formatted(date) {
    var format = activeFormat
      .replace(/ww/g, Model.isoWeekLiteral(date.getFullYear(), date.getMonth(), date.getDate()))
      .replace(/SS/g, ("0" + Math.floor(date.getMilliseconds() / 10)).slice(-2))
  ...
  Timer {
    ...
    running: true
    repeat: true
    onTriggered: root.displayDate = new Date()
  }
